...
-- Agrégation retour actif réclamations postpaid par jour et campagne
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN statut_appel = 'DECROCHE' AND a_paye = 1 THEN nd END) AS nb_decroche_et_paye,
    COUNT(DISTINCT CASE WHEN statut_appel = 'DECROCHE' AND est_actif_aujourdhui = 1 THEN nd END) AS nb_decroche_et_actif,
    COUNT(DISTINCT CASE WHEN statut_traitement = 'TRAITE' AND a_paye = 1 THEN nd END) AS nb_traite_et_paye,
...
FROM {{ ref('int_recla_parc_postpaid') }}
